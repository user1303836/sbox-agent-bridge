...
export function registerSceneTools(server: McpServer, bridge: BridgeClient): void {
  server.tool(
    "scene",
    "Read scene summary, hierarchy, and search results from the active s&box editor scene.",
    {
      action: z.enum(["summary", "hierarchy", "find"]).describe("The scene action to run."),
