...
export function registerPhysicsTools(server: McpServer, bridge: BridgeClient): void {
  server.tool(
    "physics",
    "Add physics/collider/joint components and run simple scene raycasts in the active s&box editor scene.",
    {
      action: z.enum(["add_physics", "add_collider", "add_joint", "raycast"]).describe("The physics action to run."),
      gameObjectId: z.string().optional().describe("Target GameObject id for add_physics, add_collider, or add_joint."),
