...
- `editor.wait_compile`: MCP-side wait helper that polls `editor.compile_status` until compile groups settle.
- `editor.wait_runtime`: MCP-side wait helper that polls runtime-targeted play state and scene summary until the live `GameSession` is ready.
- `editor.wait_stopped`: MCP-side wait helper that polls `editor.tabs` until no editor scene sessions are still playing.
...
The `editor.wait_*` actions are MCP-side helpers, not raw bridge commands. They repeatedly call short bridge reads so the editor frame pump keeps ticking while agents wait for compile, play, or stop transitions.
...
3. If source changed, call `editor.wait_compile`, then inspect `editor.compile_status` or `editor.feedback`.
...
6. Call `editor.wait_runtime`, then call `editor.feedback` with `targetSession: "runtime"` and the baseline `afterIndex`.
...
9. Call `editor.wait_stopped` to confirm no editor scene sessions are still playing.
10. If scene reads become empty or stale after play/stop, call `editor.open_scene` with `forceReload: true` for the saved scene path before continuing editor-scene mutations. Scratch/test scenes may also pass `discardUnsaved: true` when a stale duplicate tab blocks reload.
...
`editor.open_scene` supports `forceReload: true` to recover the editable scene, and `discardUnsaved: true` for scratch/test scenes where a stale unsaved duplicate tab blocks reload. This is a recovery path; post-transition editor-scene restoration is still needed.

## Open Work

- Capture structured live `LogEvent` entries if a stable public hook is verified for editor libraries.
- Add post-transition restoration helpers so stale duplicate scene tabs do not require manual `open_scene` recovery.
