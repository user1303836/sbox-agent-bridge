...
- Date: 2026-05-02
- s&box project: Generated fresh Minimal Game project `local.agent_bridge_mvp_fresh`, plus clean-room ARPG/boxing walkthrough scenes for gameplay smokes
...
- Latest live gate: `npm run smoke:mvp-suite`, `npm run walkthrough:arpg`, and `npm run audit:capabilities` passed on 2026-05-02. The audit reported 178 matrix rows: 134 `Verified`, 44 `Verified gap`, and 0 unverified/partial/planned/blocked rows.
...
| Custom GameResource authoring | `asset.create_resource`, `asset.list_types`, `asset.get_info` | `asset` / `create_resource` | Verified | Live `smoke:asset-resources` creates a generic `.sound` `Sandbox.SoundEvent` through `AssetSystem.CreateResource`, compiles it, and reads it back through `asset.get_info`. The bridge now treats compiled sidecars as part of overwrite handling and falls back to source-file registration for immediate read-back. |
...
| Camera capture feedback | `visual.capture_camera` | `visual` / `capture_camera` | Verified | Live IPC rendered `CameraComponent` output to a PNG under `%TEMP%/sbox-agent-bridge/captures` and returned camera metadata plus luminance stats. The ARPG clean-room walkthrough captured a generated runtime camera by GameObject id at 640x360, returned a 17 KB PNG, average luminance `0.144`, and max luminance `0.8227`. |
...
| ARPG clean-room walkthrough | Verified | `mcp-server/test/arpg-cleanroom-walkthrough.ts` creates an isolated ARPG scene and controller through the bridge, verifies character creation, Warrior/Mage skills, health/mana orbs, aggro, collision policy, inventory, loot, chest, vendor, elite, health-orb pickup, runtime test actions, and generated runtime camera capture by GameObject id. |
...
| Capability matrix status audit | Verified | `npm run audit:capabilities` parses `docs/capability-matrix.md` and fails if any row is not `Verified` or `Verified gap`. Latest audit reported 178 rows with 100% verified-or-verified-gap coverage. |
