...
| Code diagnostics beyond compile events | `editor.compile_status`, composed MCP wait helper | `editor` / `compile_status`, `editor` / `wait_compile` | Partial | Compile groups, recent diagnostics, and hotload settle waits are visible, but there is no code navigation, symbol search, generated API lookup, or analyzer surface. |
