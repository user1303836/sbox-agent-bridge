...
- `smoke:mvp-suite` is live-verified. It creates its own saved scene and runs bootstrap, MVP, asset/material, physics, sound, prefab, and capability-gap smokes against that scene, so the external tester gate no longer depends on `scenes/minimal.scene`.
...
- There is no bridge action yet for switching the open editor project. Fresh-project tests can instantiate/install a project from scripts, but the tester still has to open the generated `.sbproj` in s&box before running bridge smokes.
...
- `npm run smoke:mvp-suite` is the preferred broad external-tester gate. `smoke:mvp` remains useful as a narrower pass against an existing saved scene. The older `smoke:live` remains useful for broad regression checks but has more historical fixture assumptions.
