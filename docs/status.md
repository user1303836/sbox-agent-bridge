...
- There is no bridge action yet for creating a brand-new s&box project or switching the open editor project. Fresh-project tests still require a human-opened project with the bridge installed, but the bridge can now create a scene and save it under a new path once the project is open.
...
- `npm run smoke:mvp` is the preferred broad external-tester smoke. `smoke:bootstrap` covers fresh scene creation/save-as/reload, and `smoke:capability-gaps` covers script delete, animation helper setup, and basic particle setup. The older `smoke:live` remains useful for broad regression checks but has more historical fixture assumptions.
...
- Add first-class project bootstrap tools: create/open/switch project if feasible, scene template install, and fresh-project quickstart polish.
