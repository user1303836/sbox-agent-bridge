...
- `npm run smoke:mvp-suite` is the preferred broad external-tester gate. It passed against both the existing test project and the generated fresh Minimal Game project `local.agent_bridge_mvp_fresh`. `smoke:mvp` remains useful as a narrower pass against an existing saved scene. The older `smoke:live` remains useful for broad regression checks but has more historical fixture assumptions.
