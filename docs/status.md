...
- `smoke:capability-gaps` is live-verified. It creates/edits/deletes a scratch script with green compile read-back, verifies `SkinnedModelRenderer` plus `CitizenAnimationHelper` setup against `models/citizen/citizen.vmdl`, and verifies a basic particle stack with property read-back.
...
- `npm run smoke:mvp` is the preferred external-tester smoke. `smoke:capability-gaps` covers script delete, animation helper setup, and basic particle setup. The older `smoke:live` remains useful for broad regression checks but has more historical fixture assumptions.
