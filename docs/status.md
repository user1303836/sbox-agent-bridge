...
- Test project: Minimal Game-derived local project with clean-room ARPG and boxing bridge testbeds
...
- `visual.capture_camera` is live-verified against active cameras and generated runtime cameras selected by GameObject id. The ARPG clean-room walkthrough captured `ARPG Cleanroom Camera` at 640x360 with a 17 KB PNG and nonblack luminance stats. `asset.preview_model` is live-verified against the runtime GameSession for isolated model/material previews. Both write PNGs under `%TEMP%/sbox-agent-bridge/captures` and return luminance stats for visibility/readability checks.
...
- The clean-room ARPG walkthrough is live-verified through `npm run walkthrough:arpg`. It creates an isolated scene/controller through bridge actions, verifies character creation, Warrior/Mage skills, orbs, aggro, collision policy, inventory, loot, chest, vendor, elite, health-orb pickup, runtime test actions, and generated runtime camera capture by GameObject id.
...
- `npm run audit:capabilities` is live-verified. It parses `docs/capability-matrix.md` and fails unless every capability row is either `Verified` or `Verified gap`; the latest run reported 178 rows, 134 `Verified`, 44 `Verified gap`, and no partial/unverified/planned/blocked rows.
...
- `visual.capture_camera` captures camera output, not the editor/game viewport overlay. Runtime UI self-report is available through test actions, but generic viewport/HUD pixel capture and panel hierarchy inspection remain open. In multi-camera scenes, callers should pass `cameraComponentId` or `gameObjectId`; generated runtime camera targeting is verified when the object is attached to the live scene tree.
