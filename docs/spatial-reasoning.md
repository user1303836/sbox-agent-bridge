...
These are not enough to infer "which side is up" with confidence.
...
- `asset.inspect_model`: load a model and return model bounds, local bounds at candidate rotations, material slots, source/compiled paths, pivot/origin information, and any available source-axis metadata.
- `asset.orientation_candidates`: evaluate common orientation candidates such as identity, pitch `90`, pitch `-90`, roll `90`, roll `-90`, and yaw-preserving variants; return local/world bounds and ground offsets for each.
...
- `viewport.capture` or `asset.preview_capture`: capture small visual previews for human or vision-model confirmation when metadata confidence is low.
...
2. Inspect model bounds and orientation metadata.
3. If an override exists, use it.
4. If no override exists, place an orientation-candidate fixture with several rotations.
5. Ask for human or vision confirmation when candidates are ambiguous.
...
- Implement model inspection that exposes model bounds and source/compiled asset paths in a compact response.
- Add project-local orientation overrides.
- Teach the ARPG POC prop kit to use overrides for all generated props.
- Add a live smoke case that places one model from an override, verifies bounds, and saves/loads the scene.
