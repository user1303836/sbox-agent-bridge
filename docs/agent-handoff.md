...
- The capability gap smoke (`npm run smoke:capability-gaps`) closes the easiest implemented-but-unverified rows: scratch `script.delete`, compile diagnostics/recovery, `SkinnedModelRenderer`/`CitizenAnimationHelper`, and basic particle stack setup.
...
- Clean-room project setup: project files can be created from the local template, the bridge can be installed by script, and `scripts/start-sbox-project.ps1` can launch that project as a separate editor process with isolated IPC. There is still no in-editor bridge action to switch the current process to another project. Once a project is open with the bridge installed, `editor.project_info`, `editor.new_scene`, `editor.save_scene_as`, and `smoke:mvp-suite` cover MVP verification without relying on the ARPG scene.
